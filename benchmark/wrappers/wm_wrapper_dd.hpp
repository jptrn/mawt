...
public:
    wm_wrapper_dd(const std::vector<value_type>& s, size_t bit_depth) :
        m_time(),
        m_wm(wm_make_dd<value_type, wt_impl_type, wt_dd_impl_type>(s, bit_depth, m_time)) { }
    
    inline static const std::string name() {
        std::stringstream s;
        s << "wm_dd_" << wt_impl_type::to_string() << "_" << wt_dd_impl_type::to_string();
        return s.str();
    }
